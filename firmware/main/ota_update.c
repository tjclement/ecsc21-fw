...
static const char *REQUEST = "GET " CONFIG_OTA_WEB_PATH
                             " HTTP/1.0\r\n"
...
static EventGroupHandle_t wifi_event_group =
    0;  // FreeRTOS event group to signal when we are connected & ready to make a request
...
void graphics_show(const char *text, uint8_t percentage, bool showPercentage, bool force) {
    ...
    if (force || percentage == 0 || (percentage >= lastShownPercentage + 10)) {
        ...
        if (showPercentage) {
            ...
            driver_framebuffer_print(NULL, buffer, driver_framebuffer_getWidth(NULL) - 100,
                                     driver_framebuffer_getHeight(NULL) - 50, 2, 2, COLOR_BLACK, &roboto_12pt7b);
        }
        driver_framebuffer_print(NULL, "BADGE.TEAM", 0, driver_framebuffer_getHeight(NULL) - 15, 1, 1, COLOR_BLACK,
                                 &fairlight_8pt7b);
...
static inline uint8_t *index_crlf(uint8_t *buf, size_t buf_len) {
    while (buf_len >= 2) {
...
int mbedtls_ssl_handshake_(mbedtls_ssl_context *ssl) {
    while (1) {
...
int mbedtls_ssl_read_(mbedtls_ssl_context *ssl, unsigned char *buf, size_t len) {
    while (1) {
...
int mbedtls_ssl_write_(mbedtls_ssl_context *ssl, const unsigned char *buf, size_t len) {
    int total_len = 0;
    while (len > 0) {
...
static void badge_ota_task(void *pvParameter) {
    esp_err_t err;
    uint8_t buffer[1024];
    int buffer_len = 0;
    ...
    ESP_LOGW(TAG, "Running from partition type %d subtype %d (offset 0x%08x)", part_running->type,
             part_running->subtype, part_running->address);

    const esp_partition_t *part_update = esp_ota_get_next_update_partition(NULL);
    assert(part_update != NULL);
    ESP_LOGW(TAG, "Writing to partition type %d subtype %d (offset 0x%08x)", part_update->type, part_update->subtype,
             part_update->address);
    ...
    ESP_LOGW(TAG, "Connecting to %s:%u...", CONFIG_OTA_WEB_SERVER, CONFIG_OTA_WEB_PORT);
