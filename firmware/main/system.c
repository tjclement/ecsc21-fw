...
void logo() {
    ...
    printf("\nRunning on an Espressif ESP32 (rev. %d) with %d CPU cores, WiFi%s%s and %dMB %s flash.\r\n",
           ...
           (chip_info.features & CHIP_FEATURE_BLE) ? "/BLE" : "", spi_flash_get_chip_size() / (1024 * 1024),
...
int get_magic() {
    int magic, inv_magic;
    if (driver_rtcmem_int_read(0, &magic) != ESP_OK)
        return 0;
    if (driver_rtcmem_int_read(1, &inv_magic) != ESP_OK)
        return 0;
